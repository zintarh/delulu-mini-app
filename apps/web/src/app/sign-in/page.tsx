...
export default function SignInPage() {
  ...
  useEffect(() => {
    if (!isReady || !authenticated) return;
    if (!address || (!isSuccess && !isError)) return;
    const username = typeof existingUsername === "string" ? existingUsername.trim() : "";
    router.replace(username.length > 0 ? "/" : "/welcome");
  }, [isReady, authenticated, address, isSuccess, isError, existingUsername, router]);
  ...
  }, [isReady, authenticated, address, router]);
      ...
      <div className="w-full max-w-[360px] flex flex-col items-center gap-10">
        ...
        <div className="w-full flex flex-col gap-3">
          ...
          <a
            ...
            rel="noreferrer"
            className="w-full flex items-center justify-center gap-2.5 py-4 rounded-2xl
              bg-[#229ED9] text-white font-bold text-base
