...
function getAdminBase(pathname: string): string {
  const segment = pathname.split("/")[1] ?? "admin";
  return `/${segment}`;
}

function getBreadcrumb(pathname: string, base: string): string {
  const map: Record<string, string> = {
    [`${base}`]: "Overview",
    [`${base}/milestones`]: "Milestone Queue",
    [`${base}/markets`]: "All Markets",
    [`${base}/broadcasts`]: "Broadcasts",
    [`${base}/send-email`]: "Send Email",
    [`${base}/users`]: "Users",
  };
...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const breadcrumb = getBreadcrumb(pathname, base);
    ...
    <div className="h-screen flex overflow-hidden bg-[#f9f8f4] text-foreground">
      ...
      <aside className={cn(
        ...
        <div className="shrink-0 border-t border-white/10 p-4 space-y-2">
          ...
          {isConnected && address && (
            <p className="px-1 text-[10px] font-mono text-white/30 truncate">
          ...
          <button
            ...
            className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold text-white/40 hover:bg-white/[0.06] hover:text-white/70 transition-colors"
          >
