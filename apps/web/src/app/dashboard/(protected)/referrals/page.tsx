...
import { ReferrersTable } from "@/components/admin/referrers-table";
import {
  DashboardPage,
  DashboardTableCard,
...
export default function AdminReferralsPage() {
  ...
  const [view, setView] = useState<"referrals" | "referrers">("referrals");
  ...
  return (
    <DashboardPage className="max-w-none px-5 sm:px-7">
      ...
      <div className="mb-4 inline-flex rounded-full bg-secondary/60 p-1">
        {(
          [
            { value: "referrals", label: "Referrals" },
            { value: "referrers", label: "Who can refer" },
          ] as const
        ).map((v) => (
          <button
            key={v.value}
      ...
      {view === "referrers" ? (
        <>
          <p className="mb-5 text-sm text-muted-foreground">
            Everyone who has referred a signup. Locked = missed a milestone and today&apos;s proof
            isn&apos;t in; their invitees can&apos;t sign up and payouts are held until they post.
          </p>
          <ReferrersTable />
        </>
      ) : (
