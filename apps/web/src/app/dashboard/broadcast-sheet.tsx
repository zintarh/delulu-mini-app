  ...
  const html = `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:560px;margin:0 auto;background:#ffffff;border-radius:16px;border:1px solid #e5e7eb;overflow:hidden;">
    <div style="padding:24px 28px 18px;text-align:center;border-bottom:1px solid #e5e7eb;background:#fafafa;">
      <div style="display:inline-flex;align-items:center;justify-content:center;width:40px;height:40px;background:#1a1a19;border-radius:10px;margin-bottom:8px;">
        <span style="color:#2563eb;font-size:20px;">D</span>
      </div>
...
export function BroadcastSheet({ open, onOpenChange, delulus, isLoading }: BroadcastSheetProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/dashboard/broadcast?addresses=${encodeURIComponent(addresses)}`)
      .then((r) => r.json())
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-border px-6 py-4 shrink-0">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-delulu-blue">
          ...
          {step === "done" && result ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-5 px-6 py-12">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-delulu-blue-light">
                <CheckCircle2 className="h-8 w-8 text-delulu-blue" />
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 w-full max-w-sm">
                {[
                  { label: "Sent", value: result.sent, color: "text-delulu-blue" },
              ...
              <button
                ...
                className="rounded-lg bg-delulu-blue px-6 py-2.5 text-sm font-bold text-white hover:bg-delulu-blue/90 transition-colors"
              >
                Close
              </button>
            </div>
            ...
            <div className="flex flex-1 min-h-0 flex-col lg:flex-row">
              ...
              <div className="flex flex-col w-full lg:w-[42%] border-b lg:border-b-0 lg:border-r border-border min-h-0">
                <div className="shrink-0 px-4 pt-4 pb-3 border-b border-border space-y-2.5">
                  ...
                  />
                  {selected.size > 0 && (
                    <div className="flex items-center gap-2 rounded-lg bg-delulu-blue-light border border-delulu-blue-border px-3 py-2">
            ...
            <div className="shrink-0 flex items-center justify-between gap-3 border-t border-border px-6 py-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={handleSend}
                  disabled={!canSend}
                  className="inline-flex items-center gap-2 rounded-lg bg-delulu-blue px-4 py-2 text-sm font-bold text-white disabled:opacity-40 hover:bg-delulu-blue/90 transition-colors"
                >
