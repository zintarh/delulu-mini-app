...
async function fetchCeloPriceShared(): Promise<{
  usd: number | null;
  error: string | null;
}> {
  const now = Date.now();
  ...
  inFlight = (async () => {
    try {
      const res = await fetch("/api/prices/celo", {
        cache: "no-store",
      });
      if (!res.ok) {
      ...
      return {
        usd: cachedUsd,
        error: e instanceof Error ? e.message : "Failed to load CELO price",
      };
    } finally {
      inFlight = null;
    }
...
export function useCeloPrice(): PriceState {
  ...
  useEffect(() => {
    ...
    async function fetchPrice() {
      setIsLoading(true);
      setError(null);
      const result = await fetchCeloPriceShared();
      if (isMounted) {
        setUsd(result.usd);
