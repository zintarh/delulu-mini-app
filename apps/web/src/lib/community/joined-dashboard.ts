...
/**
 * Every active campaign a wallet has joined, with its next milestones and how
 * many it has already missed. Backs home / profile "active campaigns" and the
 * referral standing check (lib/referral/standing.ts).
 * Inverted path: graph (or DB joins) first → fetch only those campaign rows —
 * never scan every on-chain campaign in the table.
 */
export async function fetchJoinedCampaignDashboard(
  admin: SupabaseAdmin,
  address: string,
  ...
  const joinedAtMsByCampaign = new Map(
    (participantRows ?? []).map((p) => [p.campaign_id, p.joined_at ? new Date(p.joined_at).getTime() : null]),
  );
  ...
  const offChainPromise = (async () => {
    ...
    return activeOffChainRows.map((raw) => {
      ...
      return {
        ...
        missed_count: scheduled.filter((m) => {
          const joinedAtMs = joinedAtMsByCampaign.get(cid) ?? null;
          return m.is_overdue && (joinedAtMs == null || new Date(m.deadline).getTime() > joinedAtMs);
        }).length,
  ...
  const campaigns = [...onChainCampaigns, ...offChainCampaigns];
  return withAvatars ? withParticipantAvatars(campaigns) : campaigns;
}
