/**
 * Validates ForfeitMarket upgrade compatibility against an existing UUPS proxy.
 *
 * Usage:
 *   PROXY_ADDRESS=0x... \
 *   npx hardhat run scripts/validate-upgrade-forfeit-market.cjs --config hardhat.config.cjs --network celo
 */
...
async function main() {
  ...
  console.log("=== ForfeitMarket Proxy Upgrade Compatibility Validation ===\n");
  console.log("Proxy address:", proxyAddress);

  const Factory = await ethers.getContractFactory("ForfeitMarket");
